...
pub struct Config {
    ...
    // ── споры ────────────────────────────────────────────────────────────
    //
    // Все поля ниже добавлены миграцией 0.2.0. `serde(default)` обязателен:
    // без него контракт не прочитал бы собственный конфиг, сохранённый до
    // миграции. Реальные значения ставит обработчик `migrate`, и
    // `check_config` не даст сохранить нули.

    /// Кто решает споры. Отдельный адрес, в идеале мультисиг: при одном
    /// ключе "оспорить у арбитра" значит пожаловаться оператору на оператора.
    /// Пока не задан, споры решает admin.
    #[serde(default)]
    pub arbiter: Option<Addr>,
    /// Залог оспаривания. Возвращается с наградой, если оспоривший прав,
    /// и уходит в фонд доплат, если нет.
    #[serde(default)]
    pub challenge_bond: Uint128,
    /// Сколько у арбитра времени на решение. Молчание ведёт к аннулированию,
    /// а не к победе объявленного исхода: иначе неудобный спор можно было бы
    /// просто не заметить.
    #[serde(default)]
    pub arbiter_secs: u64,
    /// Сколько ждём объявления исхода после `resolve_after`, прежде чем
    /// аннулировать рынок может кто угодно. Без этого потеря ключа
    /// резолвера замораживала бы ставки навсегда.
    #[serde(default)]
...
pub struct Spec {
    ...
    /// В чём выражен `threshold`: `uluna`, `percent`, `rate`. Раньше единица
    /// подразумевалась, и порог 6000000000000 uluna читался как 6T LUNC,
    /// хотя это 6M. Необязателен ради старых рынков.
    #[serde(default)]
...
pub enum Status {
    ...
    Proposed,
    /// Исход оспорен, ждём решения арбитра. Выплаты закрыты.
    Disputed,
...
pub struct Market {
    ...
    // ── спор и итог ──────────────────────────────────────────────────────
    //
    // Добавлены миграцией 0.2.0, отсюда `serde(default)`: рынки, созданные
    // раньше, читаются с пустыми значениями.

    #[serde(default)]
    pub disputed_at: Option<u64>,
    #[serde(default)]
    pub challenger: Option<Addr>,
    /// Показание оспорившего. `reading` остаётся показанием резолвера -
    /// так после спора видно, кто что утверждал.
    #[serde(default)]
    pub challenge_reading: Option<String>,
    /// Залог оспаривания, лежащий на контракте. Обнуляется в тот момент,
    /// когда его вернули или отдали в фонд, - чтобы не отдать дважды.
    #[serde(default)]
    pub challenge_bond: Uint128,
    /// Обоснование арбитра.
    #[serde(default)]
    pub ruling: Option<String>,
    /// Почему рынок аннулирован. Раньше причина жила только в событии
    /// транзакции, и интерфейсу её было неоткуда взять.
    #[serde(default)]
    pub void_reason: Option<String>,
    /// Сгорел ли залог создателя при аннулировании.
    #[serde(default)]
    pub bad_spec: bool,
}
