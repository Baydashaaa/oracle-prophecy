...
pub enum ExecuteMsg {
    ...
    /// Предсказание: занять сторону рынка. Монеты прикладываются к сообщению.
    /// Повторное предсказание на ту же сторону увеличивает существующее.
    ///
    /// Называлось `bet` до 0.2.1. Старое имя принимается как синоним, чтобы
    /// не сломать кошельки и скрипты, собранные под прежнюю версию; сайт
    /// отправляет `predict`, и в окне кошелька слова "bet" больше нет.
    #[serde(alias = "bet")]
    Predict { market_id: u64, side: bool },
